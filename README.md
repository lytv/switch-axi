...
3. Switch Console auto-adopts credentials under an already-onboarded location. The CLI saves the working directory to `$XDG_CONFIG_HOME/switch-axi/pending-locations.json`, or `~/.config/switch-axi/pending-locations.json` when unset. Console reads this file at start. If a Console is running and exposes its local control API, the CLI also asks it to open the folder immediately. If the new folder is not onboarded yet, Console opens it from the queued path.
